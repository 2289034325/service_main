...
public interface WritingRepository {
    ...
    @Insert("insert into `writing.article` (id,lang,source,title,insert_time,deleted) values(#{id},#{lang},#{source},#{title},#{insert_time},0)")
    Integer insertArticle(WritingArticle article);

    @Update("update `writing.article` set lang=#{lang}, source = #{source},title = #{title} where id = #{id}")
